...
if ( $result ) {
  $site_title = str_replace( [ '%title%', '%duration%', '%domain%' ], [ $result['title'], $result['duration'], $_SERVER['HTTP_HOST'] ], $set['stream_title'] );
  $meta_description = str_replace( [ '%title%', '%duration%', '%domain%' ], [ $result['title'], $result['duration'], $_SERVER['HTTP_HOST'] ], $set['stream_desc'] );
  $meta_robots = $set['stream_index'];
} else {
  redirect( site_url() );
}

include 'themes/'.$set['theme'].'/header.php';
require 'themes/'.$set['theme'].'/stream.php';
